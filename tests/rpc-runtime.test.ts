...
function makeRuntimeImpl(): ImplOf<typeof RuntimeCap> {
  let bwIdCounter = 0;
  const windows = new Map<number, { label: string; bounds: { x: number; y: number; w: number; h: number }; title: string }>();
  const makeBw = (bwId: number, ctx: any) => ctx.exportCap(BrowserWindowCap, {
    focus: () => {},
    close: () => { windows.delete(bwId); },
    setBounds: (b: { x: number; y: number; w: number; h: number }) => { const w = windows.get(bwId); if (w) w.bounds = b; },
    setTitle: ({ title }: { title: string }) => { const w = windows.get(bwId); if (w) w.title = title; },
    id: () => bwId,
    label: () => windows.get(bwId)?.label ?? "",
  });
  return {
    window: (_, ctx) => ctx.exportCap(WindowCap, {
      create: (opts, c2) => {
      ...
      focus: () => {},
      close: ({ id }) => { if (id != null) windows.delete(id); },
    }),
    dialogs: (_, ctx) => ctx.exportCap(DialogsCap, {
      openFile: () => [],
      saveFile: (_, c2) => c2.exportCap({} as never, {} as never),
      showMessage: () => "primary",
    }),
    clipboard: (_, ctx) => ctx.exportCap(ClipboardCap, {
      readText: () => "clip",
      writeText: () => {},
      readBytes: () => new Uint8Array(),
      writeBytes: () => {},
    }),
    shell: (_, ctx) => ctx.exportCap(ShellCap, {
      openExternal: () => true,
      showItemInFolder: () => {},
    }),
    appName: () => "test-app",
...
describe("runtime cap dispatch", () => {
  test("appName / appVersion / theme via runtime()", async () => {
    ...
    createConnection({ transport: t2, mode: "native", origin: "test://server", runtime: makeRuntimeImpl() });
