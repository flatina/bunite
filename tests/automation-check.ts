...
async function doChecks() {
  ...
  const RESET_INPUT =
    "(function(){var i=document.getElementById('input');i.value='';i.focus();return ''})()";
  ...
  if (caps.type) {
    ...
    v.type("hi");
    await sleep(200);
    const r3 = (await v.evaluate("document.getElementById('input').value")) as {
      ok: boolean;
      value?: unknown;
    };
    ...
    v.type("안녕");
    await sleep(200);
    const r3b = (await v.evaluate("document.getElementById('input').value")) as {
      ok: boolean;
      value?: unknown;
    };
  ...
  if (caps.click) {
    ...
    if (rect.ok && rect.value && typeof rect.value === "object") {
      ...
      v.click({ x, y });
      await sleep(200);
      const r5 = (await v.evaluate("document.getElementById('log').textContent")) as {
        ok: boolean;
        value?: unknown;
      };
      ...
      ok(
        `click isTrusted matches nativeInputTrusted (${trustedExpected})`,
        log.includes(`trusted=${trustedExpected}`),
        log,
      );
      ...
      await sleep(200);
      const rm = (await v.evaluate("document.getElementById('log').textContent")) as {
        ok: boolean;
        value?: unknown;
      };
      ok(
        "click with ctrl modifier → ctrl=true in event",
        typeof rm.value === "string" && rm.value.includes("ctrl=true"),
        rm.value,
      );
      ...
      await sleep(300);
      const rd = (await v.evaluate("document.getElementById('log').textContent")) as {
        ok: boolean;
        value?: unknown;
      };
      ok(
        "click clickCount=2 → [dblclick] in log",
        typeof rd.value === "string" && rd.value.includes("[dblclick]"),
        rd.value,
      );
    } else {
  ...
  if (caps.resolveAndClick) {
    ...
    await sleep(200);
    const r7 = (await v.evaluate("document.getElementById('log').textContent")) as {
      ok: boolean;
      value?: unknown;
    };
    ok(
      "resolveAndClick fires [click] on page",
      typeof r7.value === "string" && r7.value.includes("[click]"),
      r7.value,
    );
    if (rc.ok) {
      ok(
        "resolveAndClick isTrustedEvent matches backend expectation",
        typeof rc.isTrustedEvent === "boolean",
        { isTrustedEvent: rc.isTrustedEvent },
      );
    }
    // missing selector → not_found
    const rcMissing = await v.resolveAndClick({ surfaceId: v.id, selector: "#does-not-exist" });
    ok(
      "resolveAndClick(missing) → not_found",
      rcMissing.ok === false && rcMissing.code === "not_found",
      rcMissing,
    );
  }
  ...
  if (caps.press) {
    await v.evaluate(
      "(function(){document.getElementById('log').textContent='';document.body.focus();return ''})()",
    );
    v.press("Enter");
    await sleep(150);
    const r6 = (await v.evaluate("document.getElementById('log').textContent")) as {
      ok: boolean;
      value?: unknown;
    };
  ...
  if (caps.scroll) {
    await v.evaluate(
      "(function(){document.getElementById('log').textContent='';window.scrollTo(0,0);return ''})()",
    );
    v.scroll({ dx: 0, dy: 200, x: 100, y: 100 });
    await sleep(200);
    const r7 = (await v.evaluate("window.scrollY")) as { ok: boolean; value?: unknown };
    ok(
      "scroll dy=200 → scrollY > 0",
      r7.ok === true && typeof r7.value === "number" && (r7.value as number) > 0,
      r7,
    );
  }
  ...
  if (caps.screenshot) {
    ...
    if (shot.ok) {
      ...
      ok(
        `screenshot png → ${shot.data.byteLength} bytes (${outPath})`,
        shot.data.byteLength > 1000,
      );
    } else {
  ...
  await sleep(3500);
  ok("load-fail event fires on unresolved host", loadFailFired, loadFailFired ? loadFailUrl : "");
  ...
  if (caps.mouse) {
    await v.evaluate(
      "(function(){window.__mt=[];['mousedown','mousemove','mouseup'].forEach(function(t){document.addEventListener(t, function(e){window.__mt.push(t+':'+e.isTrusted)});});return ''})()",
    );
    ...
    ok(
      "mouse drag down+up sequence reaches listeners (isTrusted=true)",
      /mousedown:true.+mouseup:true/.test(seq),
      seq,
    );
  }
  ...
  if (caps.dialogs) {
    const dialogCases: Array<{
    ...
    }> = [
      {
        ...
        accept: true,
        text: "",
        expr: "(function(){window.alert('a');return 'ok'})()",
        expect: "ok",
      },
    ...
    for (const c of dialogCases) {
      ...
      ok(
        `dialog ${c.kind} accept=${c.accept} → ${JSON.stringify(c.expect)}`,
        r.ok === true && r.value === c.expect,
        r,
      );
    }
    ok(
      "dialog kinds observed in order",
      seen.length === dialogCases.length && seen.every((k, i) => k === dialogCases[i].kind),
      seen,
    );
  }
